...
void renderer::image_set(volume_block *b)
{
  ...
  sdf_generation_init.execute({evenness(b->m_voxel_count_x, 8), evenness(b->m_voxel_count_y,8), evenness(b->m_voxel_count_z, 8)}, {8,8,8}, reference_volume, sdf);
  ...
  for (int i = 0; i < 1000; ++i) {
    ...
    if (i % 2 == 0)
      sdf_generation_initialization.execute({evenness(b->m_voxel_count_x, 8), evenness(b->m_voxel_count_y,8), evenness(b->m_voxel_count_z, 8)}, {8,8,8}, sdf, sdf_volume_pong, atomic_add_buffer);
    else
      sdf_generation_initialization.execute({evenness(b->m_voxel_count_x, 8), evenness(b->m_voxel_count_y,8), evenness(b->m_voxel_count_z, 8)}, {8,8,8}, sdf_volume_pong, sdf, atomic_add_buffer);
