...
__kernel void tf_fetch_min_max(__read_only image3d_t reference_volume, __global int *stats)
{
  ...
  float3 grad_value = gradient_prewitt_nn(reference_volume, pos);
  float grad_lenght = length(grad_value);
  ...
  atomic_min(&stats[MIN_GRADIENT], grad_lenght);
  atomic_max(&stats[MAX_GRADIENT], grad_lenght);
}
  ...
  int width, int height, __global int *stats)
{
  ...
  int max_gradient = stats[MAX_VALUE];
  int min_gradient = stats[MIN_VALUE];
  ...
  int2 relative_point = {
    ...
    round(((float)(grad_lenght - min_gradient)/gradiant_range)*height)
  };
